...
func _process(_delta):
	if (inventory["carbohydrates"] - task_tracker) > 0:
		get_parent()._task_create(get_path(),"/root/Main/Gameworld/MainBuilding","carbohydrates")
		task_tracker += 1
...
func _place_building():
	...
	if (
		...
		get_parent().get_node("/root/Main/Gameworld/MainBuilding").inventory["dirt"] -= 60
		get_parent().get_node("/root/Main/Gameworld/MainBuilding").inventory["water"] -= 50
...
func _on_timer_timeout():
	if inventory["carbohydrates"] < 3:
		inventory["carbohydrates"] += 1
