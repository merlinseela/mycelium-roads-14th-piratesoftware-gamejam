...
func _process(_delta):
	if (inventory["phosphorus"] - task_tracker) > 0:
		get_parent()._task_create(get_path(),"/root/Main/Gameworld/MainBuilding","phosphorus")
		task_tracker += 1
...
func _on_timer_timeout():
	if inventory["phosphorus"] < 3:
		inventory["phosphorus"] += 1
